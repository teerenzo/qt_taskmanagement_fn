...
const TaskDetailsScreen = () => {
    ...
    const {id}=useParams()
    console.log(id)
    const selectedTask= tasks.find((e)=>e.id===parseInt(id))

    const assigneesLi= users&&users.filter((user)=>selectedTask.assignees.find(e=>e===user.id))

    console.log("ggg",assigneesLi)


  const token = localStorage.getItem('token');
    const [assignees, setAssignees] = useState(assigneesLi||[]); 
    ...
    const [files, setFiles] = useState(selectedTask.attachments||[]);
    const [projects, setProjects] = useState(selectedTask.projects||[]);
    const [priority, setPriority] = useState(selectedTask.priority);
    const [startDate, setStartDate] = useState(selectedTask.startDate? new Date(Date.parse(selectedTask.startDate)): "");
    const [endDate, setEndDate] = useState(selectedTask.endDate? new Date(Date.parse(selectedTask.endDate)): "");
  ...
  console.log(e);
    ...
    setIsLoading(true)
        ...
        var config = {
            ...
            url: `${process.env.REACT_APP_BACKEND_URL}/tasks/update/${id}`,
            headers: {
